...
class Visualizer(object):
    ...
    def __init__(self, freq, inside_freq, vis_dir, renderer, verbose, device='cuda:0',args=None):
        self.freq = freq
        self.device = device
    ...
    def vis(self, idx, iter, gt_depth, gt_color, c2w_or_camera_tensor, c,
        ...
        with torch.no_grad():
            if (idx % self.freq == 0) and (iter % self.inside_freq == 0):
                ...
                plt.clf()
                # Generate just output image
                plt.figure()
                plt.imshow(color_np, cmap="plasma")
                plt.savefig(
                    f'{self.vis_dir}/{idx:05d}_{iter:04d}_jstColr.jpg', bbox_inches='tight', pad_inches=0.2)
                plt.clf()
                
                # Render background if we have one
                if not self.args is None and self.args.bg_sphr:
                    depth, uncertainty, color = self.renderer.render_img(
                    c,
                    decoders,
                    c2w,
                    ...
                    axs[1].set_title('Generated BckGrnd RGB')
                    axs[1].set_xticks([])
                    axs[1].set_yticks([])
                    plt.subplots_adjust(wspace=0, hspace=0)
                    directory = f'{self.vis_dir}/backg/'
